...
```bash
$ make package
$ helm install mender ./mender-2.6.4.tgz
```
...
```bash
$ make package
$ helm install --name mender ./mender-2.6.4.tgz
```
...
```bash
$ helm install my-release -f values.yaml ./mender-2.6.4.tgz
```

or using `helm2`:

```bash
$ helm install --name my-release -f values.yaml ./mender-2.6.4.tgz
```
...
| `api_gateway.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `deployments.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `device_auth.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `gui.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `inventory.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `tenantadm.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `useradm.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `workflows.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `create_artifact_worker.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `auditlogs.image.tag` | Docker image tag | `mender-2.6.4` |
...
| `deviceconnect.image.tag` | Docker image tag | `mender-2.6.4` |
