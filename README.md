...
```bash
$ make package
$ helm install mender ./mender-2.6.1.tgz
```
...
```bash
$ make package
$ helm install --name mender ./mender-2.6.1.tgz
```
...
```bash
$ helm install my-release -f values.yaml ./mender-2.6.1.tgz
```

or using `helm2`:

```bash
$ helm install --name my-release -f values.yaml ./mender-2.6.1.tgz
```
...
| `api_gateway.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `deployments.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `device_auth.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `gui.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `inventory.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `tenantadm.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `useradm.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `workflows.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `create_artifact_worker.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `auditlogs.image.tag` | Docker image tag | `mender-2.6.1` |
...
| `deviceconnect.image.tag` | Docker image tag | `mender-2.6.1` |
