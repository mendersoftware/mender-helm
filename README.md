...
```bash
$ make package
$ helm install mender ./mender-2.6.3.tgz
```
...
```bash
$ make package
$ helm install --name mender ./mender-2.6.3.tgz
```
...
```bash
$ helm install my-release -f values.yaml ./mender-2.6.3.tgz
```

or using `helm2`:

```bash
$ helm install --name my-release -f values.yaml ./mender-2.6.3.tgz
```
...
| `api_gateway.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `deployments.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `device_auth.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `gui.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `inventory.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `tenantadm.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `useradm.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `workflows.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `create_artifact_worker.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `auditlogs.image.tag` | Docker image tag | `mender-2.6.3` |
...
| `deviceconnect.image.tag` | Docker image tag | `mender-2.6.3` |
